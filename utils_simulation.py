...
def generate_obstacles(p, heightObs, robotRadius):
    ...
    numObs = 20+np.random.randint(0,21) # 30 
    # radiusObs = 0.15
    massObs = 0
    visualShapeId = -1
    ...
    for obs in range(numObs):
        ...
        colIdxs[obs] = p.createCollisionShape(p.GEOM_CYLINDER,radius=(0.20 - 0.05)*np.random.random_sample(1)+0.05,height=heightObs)
